...
results <- foreach(i = 1:100, .combine = rbind) %do% {
  sample_data <- rexp(50, rate = 1) # Exponential with mean 1 (rate = 1/mean)
...
# Create an iterator for normal random numbers: 3 vectors, each containing 5 values
rand_iter <- irnorm(n = 5, mean = 0, sd = 1)

# Use foreach to find the maximum in each vector
max_values <- foreach(vec = rand_iter, .combine = c) %do% {
  max(vec)
}
...
# Define task: Generate 3 vectors of 5 normal random numbers and find max
task <- function() {
  max(rnorm(5))
}
...
# Export the task function to worker nodes
